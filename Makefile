...
$(BUILD_DIR)/test_cm_surface: tests/test_cm_surface.cpp \
		$(BUILD_DIR)/liboneshotsea.a
	$(CXX) $(CPPFLAGS) $(CXXFLAGS) $< $(BUILD_DIR)/liboneshotsea.a \
		$(LDFLAGS) $(LDLIBS) -o $@
...
$(BUILD_DIR)/validate_p125_cap_one_tail: \
		tools/validate_p125_cap_one_tail.cpp \
		$(BUILD_DIR)/liboneshotsea.a
	$(CXX) $(CPPFLAGS) $(CXXFLAGS) $< $(BUILD_DIR)/liboneshotsea.a \
		$(LDFLAGS) $(LDLIBS) -o $@
...
test-profile-classical-direct-cohort: $(BUILD_DIR)/oneshotsea \
	...
	ONESHOTSEA_BINARY=$(BUILD_DIR)/oneshotsea \
		ONESHOTSEA_COHORT_PROFILER=$(BUILD_DIR)/profile_classical_direct_cohort \
		python3 tests/test_profile_classical_direct_cohort.py -v

test-poly-square: $(BUILD_DIR)/test_poly_square
	./$(BUILD_DIR)/test_poly_square

test-poly-ab-summary:
	python3 tests/test_poly_ab_summary.py -v
...
test-performance-artifacts: test-p125-direct-atkin-mitm-audit test-p125-certified-atkin-singleton-audit test-p125-cap-one-direct-tail-audit test-p125-pre-smooth-direct-tail-audit test-p125-direct-batched-interpolation-audit test-p125-direct-atkin-frobenius-reuse-audit test-p125-direct-frobenius-composition-audit test-p125-direct-frobenius-x-window-audit test-p125-direct-hybrid-reduction-audit
