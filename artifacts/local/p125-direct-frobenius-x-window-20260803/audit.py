#!/usr/bin/env python3
"""Authenticate the p125 specialized Frobenius-X-window evidence."""
...
def main():
    ...
    equal(result["schema"],
          "oneshotsea.p125-direct-frobenius-x-window.v1",
          "result schema")
    ...
    require(generation_reduction > 0,
            "retained run does not expose the favorable control drift")
    equal(comparison["general_speedup_inference_allowed"], False,
          "generalized timing claim")
    validate_production(result, prime)

    equal(result["method"]["generic_quotient_ring_operations"],
          478, "generic quotient-ring operation count")
    equal(result["method"]["specialized_quotient_ring_operations"],
          452, "specialized quotient-ring operation count")
    equal(result["method"]["quotient_ring_operations_saved"],
          26, "deterministic quotient-ring operation saving")
    equal(result["claims"], {
        "frobenius_x_odd_powers_constructed_as_monomials": True,
        "frobenius_x_window_increased_at_level_89": True,
        "powmod_result_changed": False,
    ...
    print(json.dumps({
        "schema": "oneshotsea.p125-direct-frobenius-x-window-audit.v1",
        "ok": True,
