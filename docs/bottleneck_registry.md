...
| P0 | Production certificate search | Authenticated identities have soundly rejected every unique index 0--205. Commit `a3cc79f` covered 177--205 with 29 sound rejections, 13 full point counts, and zero certificates. Index 206 then stopped fail-closed at the implementation trace-set cap and remains unconsumed; even cap 64 is insufficient. | Consume singleton index 206 under an authenticated higher-cap identity, then resume cap-16 production at 207; never count either implementation-limit record as a rejection |
