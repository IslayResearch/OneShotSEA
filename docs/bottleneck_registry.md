...
|---|---|---|---|
| P0 | SEA cost per admitted curve | Threshold-32 exact Karatsuba, retained source state, exact-cost polynomial windows, and polynomial-subring eigen powers are committed. Specialized three-square polynomial squaring reduced a full retained-source p125 replay by 1.07060x SEA; its follow-up in-place cross-term accumulation improved a bounded same-input p125 SEA wall by another 1.07836x with all 42 projections unchanged. | Integrate the tested addmul square leaf before freezing the next production binary; measure its realized share in the new X1(27) production mix |
| P0 | Production certificate search | The durable frontier remains 246. The final v1 X1(27) attempt failed closed at index 246 with 5,542 exact and 791 effective traces, leaving its checkpoint unchanged. An isolated v2 prototype extended the fixed tail through ell 37, reduced both complete sets to 2, and soundly rejected all four orders; combined native/Magma working-tree test-all passed, but this is not committed-build evidence. | Commit v2, freeze the binary, replay 246 soundly under that final identity, then relaunch sound-only X1(27) at 247 |
...
| Implementation-limit cursor safety | `sea_level_limit` and `no_rational_weber_lift` checkpoint/report without advancing by default; the explicit skip remains heuristic and schedule-bound. Index 206 has a corrected frozen final-binary recovery. At index 246, v1 correctly stopped without advancing; a separate v2 prototype recovered it soundly through ell 37, while the v1 checkpoint remains unchanged and the v2 final-build gate remains open. |
