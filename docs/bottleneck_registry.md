...
| P0 | Production certificate search | Commit `c5de573` soundly rejected unique indices 9--11 and advanced its authenticated cursor to 12. Across unique indices 0--11, 272 trace-candidate order values were exactly screened and no certificate exists. Benchmark-only indices 12--21 also found zero certificates but do not advance that authenticated cursor. | Commit and test the exact trace-prior policy, then start a fresh X1(11)-point-four identity at 12; do not resume a pre-policy or benchmark checkpoint |
...
| P1 | Certificate opportunity model and family selection | The reproducible Dickman--Mertens model gives X1(11) cyclic-44/group-order-88 multipliers of 1.1775x/1.2394x over full-E[2] Weber. The completed same-build n=10 family A/B measured 294.57 s X1 versus 306.19 s Weber, or 1.03945x X1 wall throughput; both runs found zero certificates and used different curve distributions. Combining throughput only with the conservative cyclic model gives 1.22395x modeled opportunity rate, not observed yield. | Use X1(11) point-four after its new schedule-bound trace-prior policy passes the clean gate; retain zero-certificate and model caveats in production reporting |
