...
|---|---|---|---|
| P0 | SEA cost per admitted curve | Threshold-32 exact Karatsuba, retained source state, exact-cost polynomial windows, and polynomial-subring eigen powers are committed. Specialized three-square polynomial squaring then reduced a full retained-source p125 replay by 1.07060x SEA and 1.06837x wall, with all 55 projections unchanged. | Deploy the tested square build from the durable frontier; profile the new production mix before another arithmetic change |
| P0 | Production certificate search | The durable frontier is 246. Indices 0--205 and 207--245 have sound production outcomes; the old checkpoint retains index 206 as an explicit heuristic skip, while a separate replay under final commit `976924b3aa2148f62ceae11948824d8aed5a41bb` and frozen binary SHA-256 `67a85ad69d176f8602af5e177819709f86893a32939691b2b4ca43fc8d7c7a70` reduced 45,948/9,189 exact/effective candidates to 14/14 and soundly rejected all orders. | Start sound-only X1(27) production at 246 with fallback on and incomplete skip off |
...
| Implementation-limit cursor safety | `sea_level_limit` and `no_rational_weber_lift` checkpoint/report without advancing by default; the explicit skip remains heuristic and schedule-bound. The retained-state exact-Schoof prototype recovered index 206 without a second Weber pass, and the corrected frozen final-binary replay reproduced the complete mathematical projection and sound rejection. The historical production checkpoint remains unchanged. |
