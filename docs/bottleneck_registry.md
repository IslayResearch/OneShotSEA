...
|---|---|---|---|
| P0 | SEA cost per admitted curve | Threshold-32 exact Karatsuba, retained source state, and exact-cost polynomial windows are committed. Production eigenvalue bases lie in the zero-y polynomial subring; delegating them to the same window reduced a full retained-source p125 replay by 1.05493x and its eigenvalue stage by 1.17333x. | Deploy the committed polynomial-subring build from the durable frontier; profile the new production mix before another arithmetic change |
| P0 | Production certificate search | Authenticated identities have soundly rejected every unique index 0--176. Commit `85d289a` covered 146--176 with 31 sound rejections, 13 full point counts, and zero certificates; its immutable checkpoint is `next_index=177`. The arithmetic change deliberately gets a new build identity and fresh range rather than loading that checkpoint. | Start the committed polynomial-subring X1(11) point-four, cap-16 identity at global index 177; preserve prior artifacts and do not replay or double-count 0--176 |
