...
|---|---|---|---|
| P0 | SEA cost per admitted curve | Threshold-32 exact Karatsuba cut current-path warm curve work on indices 8/9 from 329.264/254.163 s to 110.340/90.010 s, with complete canonical projections identical. Generator-retained source state removes redundant root-specialization orbits. Exact-cost width-at-most-five polynomial exponentiation then improved paired retained-source p125 SEA by 1.16085x and modular roots by 1.33293x; the independent binary path and exact projections are retained. | Deploy the committed windowed build from the durable frontier; profile the new production mix before another arithmetic change |
| P0 | Production certificate search | Authenticated identities have soundly rejected every unique index 0--145. Commit `9655670` covered 90--145 with 56 sound rejections, 32 full point counts, and zero certificates; its immutable checkpoint is `next_index=146`. The arithmetic change deliberately gets a new build identity and fresh range rather than loading that checkpoint. | Start the committed windowed X1(11) point-four, cap-16 identity at global index 146; preserve prior artifacts and do not replay or double-count 0--145 |
