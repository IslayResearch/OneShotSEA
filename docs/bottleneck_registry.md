...
| P0 | Production certificate search | The durable frontier is 246: indices 0--205 and 207--245 have sound outcomes, while the old checkpoint records index 206 as an explicit heuristic incomplete skip. A patch-identified prototype retried 206 with skip disabled and reduced 45,948/9,189 exact/effective candidates to 14/14 via exact Schoof residues, then soundly rejected all orders. | Land and fully test the fallback, freeze the final binary, replay 206 soundly under that committed identity, then start X1(27) production at 246 with fallback on and incomplete skip off |
...
| Implementation-limit cursor safety | `sea_level_limit` and `no_rational_weber_lift` checkpoint/report without advancing by default; the explicit skip remains heuristic and schedule-bound. The retained-state exact-Schoof prototype recovered index 206 as a sound rejection without a second Weber pass, but committed-build recovery still requires a frozen final-binary replay. |
