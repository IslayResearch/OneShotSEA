...
| P0 | Production certificate search | Authenticated identities have soundly rejected every unique index 0--205. Commit `a3cc79f` covered 177--205 with 29 sound rejections, 13 full point counts, and zero certificates. Index 206 then stopped fail-closed with 9,189 candidates after every available level; raising the cap cannot make its trace unique. | Deploy the explicit schedule-bound incomplete-skip policy at 206, count it only as a heuristic false-negative-capable skip, and continue cap-16 production without relabeling it a sound rejection |
