...
| P0 | Production certificate search | Unique indices 0--11 were soundly rejected under earlier authenticated identities. Commit `1e84475` then durably processed X1(11) point-four indices 12--59: 48 sound early rejections, 26 full point counts, and zero certificates. Its authenticated checkpoint is `next_index=60`; the retained-source schedule change deliberately makes that checkpoint incompatible. | Start the committed retained-source identity at global index 60; preserve the old artifacts as evidence and do not replay or double-count 12--59 |
