...
|---|---|---|---|
| P0 | SEA cost per admitted curve | Threshold-32 exact Karatsuba, retained source state, exact-cost polynomial windows, polynomial-subring eigen powers, specialized three-square polynomial squaring, and in-place cross-term accumulation are committed in `43ca2d7`. A later smooth-coordinator candidate has one indicative 1.0886x same-range result, but the repeat was heavily contaminated and is not promotion evidence. | Run a clean bracketed B/A/A/B coordinator gate after the unrelated ten-core GP workload clears; require strict `>1.05x` before changing production identity |
| P0 | Production certificate search | The frozen `43ca2d7` sound-only X1(27) worker durably advanced from 247 through 658: 412 curves attempted, 412 sound smoothness rejections, 209 full point counts, zero heuristic skips, and zero certificates. Checkpoint `next_index` is 659. | Resume the exact frozen checkpoint at 659 after the clean coordinator decision; keep v2 fallback on and incomplete skip off |
